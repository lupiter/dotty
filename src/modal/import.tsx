...
export function Import(
  props: ModalContentProps & {
    onImport: (data: string, size: Size, title: string) => void;
  }
) {
  const [state, setState] = useState<ImportState>({ ready: false });
  const fileId = useId();
  ...
  return (
    <>
      <h1 className={modalContentsStyles.header}>Import File</h1>
